...
const MainLayout = ({ children }) => {
    ...
    return (
        ...
        <div className="wrapper d-flex flex-column" style={{ minHeight: '100vh' }}>
            ...
            <nav className="main-header navbar navbar-expand navbar-white navbar-light m-0" style={{ width: '100%', zIndex: 1000, minHeight: 56 }}>
                ...
                <ul className="navbar-nav" style={{ marginLeft: 'auto' }}>
                    <li className="nav-item dropdown">
                        <a className="nav-link" data-toggle="dropdown" href="#" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                >
                    ...
                    >
                        ...
                        <span className="brand-text" style={{ lineHeight: 1.1 }}>
                            <span style={{ display: 'block', color: '#2f4f5b', fontSize: 19, fontWeight: 800 }}>
                                BaseCoreSales
                            </span>
                            <small style={{ color: '#7b8794', fontSize: 12, fontWeight: 600 }}>
                                Admin Panel
                            </small>
                        </span>
                        )}
                    ...
                    <div className="sidebar" style={{ padding: '14px 12px' }}>
                        <nav className="mt-2">
                            <ul className="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu">
                                {renderNavLink('/dashboard', 'fas fa-tachometer-alt', 'Dashboard')}
                                {renderNavLink('/products', 'fas fa-shopping-cart', 'Products')}
                                {renderNavLink('/categories', 'fas fa-tags', 'Categories')}
                                {renderNavLink('/coupons', 'fas fa-ticket-alt', 'Coupons')}
                                {renderNavLink('/users', 'fas fa-users', 'Users')}
                                {renderNavLink('/orders', 'fas fa-file-invoice-dollar', 'Orders')}
                            </ul>
                        </nav>
                    </div>
                </aside>
